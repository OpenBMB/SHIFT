...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.ffn_output_gate = getattr(config, 'ffn_output_gate', False)
...
class LlamaForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        if getattr(self.config, 'ffn_output_gate', False):
            ffn_gate_params = {
                name for name, _ in self.named_parameters()
                if 'ffn_gate' in name
            }
            not_loaded = ffn_gate_params - (loaded or set())
            if not_loaded:
                for layer in self.model.layers:
                    if hasattr(layer, 'ffn_gate'):
                        nn.init.zeros_(layer.ffn_gate.weight)
                        nn.init.zeros_(layer.ffn_gate.bias)
            if loaded is not None:
                loaded = loaded | ffn_gate_params
